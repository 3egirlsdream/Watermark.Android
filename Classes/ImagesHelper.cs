...
namespace JointWatermark.Class
{
    public class ImagesHelper
    {
        ...
        /// </summary>
        /// <returns></returns>
        public Task<SixLabors.ImageSharp.Image> MergeWatermark(Stream stream, Stream stream2, ImageProperties properties, bool isPreview = false)
        {
            
            return CreateWatermark(stream, properties, isPreview).ContinueWith<SixLabors.ImageSharp.Image>(t =>
            {
                using (var img = SixLabors.ImageSharp.Image.Load(stream2))
                {
                    //旋转图片
        ...
        private void DrawCharacterWord(ImageProperties properties, SixLabors.ImageSharp.Image img, MauiApp3.Class.CharacterWatermarkProperty word)
        {
            var family = SetFamily(word.FontFamily).Item1.Value;
            var fontStyle = word.FontStyle;
            SixLabors.Fonts.Font font = family.CreateFont(word.FontSize * FontXS, SixLabors.Fonts.FontStyle.Regular);
        ...
        /// </summary>
        /// <returns></returns>
        public Task<Image<Rgba32>> MergeWatermarkPreview(Stream stream, ImageProperties properties, bool isPreview = false)
        {
            return CreateWatermark(stream, properties, isPreview).ContinueWith(t =>
            {
        ...
        /// </summary>
        /// <returns></returns>
        public Task<Image<Rgba32>> CreateWatermark(Stream stream, ImageProperties properties, bool isPreview = false)
        {
            return Task.Run<Image<Rgba32>>(() =>
            {
                ...
                using (var img = SixLabors.ImageSharp.Image.Load(stream))
                {
                    ...
                    SixLabors.ImageSharp.Image logo;
                    ...
                    //            logo = SixLabors.ImageSharp.Image.Load(Bytes);
                    //        }
                    //    }
                    //    else
                    //    {
                    //        logo = SixLabors.ImageSharp.Image.Load(p);
                    //    }
                    //}
                    ...
                    SixLabors.Fonts.Font font = familyBold.CreateFont(fontSize, SixLabors.Fonts.FontStyle.Bold);
                    ...
                    var lStart = new SixLabors.ImageSharp.PointF(Params.X - (int)(oneSize.Width * 0.6), (int)(0.5*(wm.Height - logo.Height)));
                    var lEnd = new SixLabors.ImageSharp.PointF(lStart.X, wm.Height - lStart.Y);
        ...
        public Task<Image<Rgba32>> CreateWatermark(ImageConfig config)
        {
            return Task.Run(() =>
            {
                ...
                using (var img = new Image<Rgba32>(1920, 1080))
                {
                    ...
                    //{
                        logo = SixLabors.ImageSharp.Image.Load("dotnet_bot.png");
                    //}
                    //else
                    //{
                    //    logo = SixLabors.ImageSharp.Image.Load(p);
                    ...
                    //右侧F ISO MM字体参数
                    float fontSize = 31 * fontxs;
                    SixLabors.Fonts.Font font = family.CreateFont(fontSize, SixLabors.Fonts.FontStyle.Bold);
                    ...
                    //绘制第右侧一行文字
                    var start = w - TextSize.Width - padding_right.Width;
                    var Params = new SixLabors.ImageSharp.PointF(start, (int)(0.3 * h));
                    ...
                    var polygon = new SixLabors.ImageSharp.Drawing.RegularPolygon(0, 0, w, Diagonal((int)h, w));
                    wm.Mutate(c => c.Fill(SixLabors.ImageSharp.Color.ParseHex("#FFF"), polygon));
                    return wm;
        ...
        //{
        //    var bmp = new Bitmap(image.Width, image.Height, image.Metadata.HorizontalResolution, image.Metadata.VerticalResolution, SixLabors.ImageSharp.PixelFormats.Bgra32, null);
        ...
        /// <returns></returns>
        public ImageProperties ReadImage(Stream stream, string filenames, string logoName)
        {
            var i = new ImageProperties(filenames, filenames.Substring(filenames.LastIndexOf(Global.SeparatorChar) + 1));
            var meta = Global.GetThumbnailPath(stream);
